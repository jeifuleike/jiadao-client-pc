<template>
  <Form ref="formCom" :validation-schema="mySchema" v-slot="{ errors }" class="xtx-form" autocomplete="off">
    ...
    <div class="xtx-form-item">
      <div class="field">
        <i class="icon iconfont icon-lock"></i>
        <Field :class="{ err: errors.rePassword }" v-model="form.rePassword" name="rePassword" class="input" type="password"
          placeholder="请确认密码" />
      </div>
...
import { addMobileUser, QQuserRegister, userRegister } from '@/api/user'
    ...
    const send = async () => {
      ...
      if (valid === true) {
        ...
        if (time.value === 0) {
          // 没有倒计时才可以发送
          try {
            await addMobileUser(form.mobile)
    ...
    const submit = async () => {
      ...
      if (valid) {
        try {
          let data = {}
          if (props.unionId) {
            data = await QQuserRegister({ unionId: props.unionId, ...form })
          } else {
            console.log(form)
            data = await userRegister({ ...form })
          }
          // 实现和之前登录一样的逻辑
          // 1. 存储用户信息
